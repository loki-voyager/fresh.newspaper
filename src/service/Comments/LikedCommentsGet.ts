...
const LikedCommentsGet = async ({
  username,
  page,
  limit,
  search,
}: RatedCommentsProps) => {
  const res = await fetch(`http://localhost:3000/api/LikedCommentsGet`, {
    cache: "no-cache",
    method: "POST",
    headers: {
  ...
  if (!res.ok) {
    throw new Error(
      `Failed to get liked comments. Error code: ${res.status}, Message: ${res.statusText}`
    );
  }
